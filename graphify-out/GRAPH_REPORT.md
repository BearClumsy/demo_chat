...
## Corpus Check
- 201 files · ~60,560 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1234 nodes · 1860 edges · 144 communities (90 shown, 54 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 163 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `48df215e`
...
Cohesion: 0.06
Nodes (41): ChatController, GrantedAuthority, Flux, Mono, PostMapping, ResponseEntity, SendMessageResponse, ServerSentEvent (+33 more)

### Community 1 - "Obsidian Style-Settings Plugin (minified)"
Cohesion: 0.04
Nodes (32): addSettingChild(), bc(), bl(), Cl(), de(), Dl(), ea(), ec() (+24 more)
...
Cohesion: 0.18
Nodes (31): Aa(), ac(), At(), Bi(), c(), Cr(), dc(), Ee() (+23 more)
...
Cohesion: 0.11
Nodes (26): DialogueState, ChatHistory, Document, Flux, IntentDefinition, List, Mono, SendMessageResponse (+18 more)

### Community 9 - "Groundedness Check & Intent JSON Validation"
Cohesion: 0.07
Nodes (30): ApplicationArguments, ApplicationRunner, ConfigurableApplicationContext, DemoChatApplication, String, Document, IntentDefinition, IntentDefinitionRegistry (+22 more)

### Community 10 - "User Persistence, Flyway & R2DBC Tests"
Cohesion: 0.23
Nodes (15): Daily Log 2026-08-13, Daily Log 2026-08-20, User Feature, SecurityUserDetailsService, User R2DBC entity, UserPrincipal, UserRepository (R2dbcRepository), UserService (+7 more)
...
Cohesion: 0.06
Nodes (32): AnswerGenerationService, ChatHistoryRepository, DialogueStateRepository, IntentClassificationService, KnowledgeRetrievalService, AssembledPrompt, ChatClient, IntentDefinition (+24 more)
...
Cohesion: 0.07
Nodes (37): ALB (SSE-compatible long-lived connection routing), Amazon Bedrock (LLM + embeddings), ECS Fargate (Spring Boot host), Amazon MSK (Kafka, planned), Self-managed Qdrant on EC2/ECS, RDS for PostgreSQL, Bedrock Titan embedding model (amazon.titan-embed-text-v2:0, 768d), Boolean (+29 more)
...
Cohesion: 0.33
Nodes (10): Amazon Keyspaces (Cassandra-compatible), Chat Feature, ChatHistory Cassandra entity, ChatHistoryRepository, ChatMessage Cassandra UDT, chat_history (Cassandra table), dialogue_state (Cassandra table), Dialogue Session Model (+2 more)

### Community 23 - "Client Chat UI & User Endpoints"
Cohesion: 0.20
Nodes (11): AuthPage (signup + login form), chatApi (hand-rolled SSE frame parser over fetch), ChatWindow / MessageBubble, StartChatForm, useChatStream hook, GET /api/users/{id} Endpoint, POST /api/users Endpoint, Moved UserController to user Package (+3 more)
...
Cohesion: 0.18
Nodes (10): Output-side groundedness guardrail, Example of an assembled prompt, Handling "sensitive" messages, Input-side guardrails, Output-side guardrails, Principle behind assembling the final prompt, Prompt Engineering and Guardrails, Related documents (+2 more)

### Community 30 - "Intent Definition Registry"
Cohesion: 0.14
Nodes (11): ChatServiceValidateParticipantIdsTest, Collection, IntentDefinition, Optional, String, BeforeEach, Test, ObjectMapper (+3 more)
...
Cohesion: 0.22
Nodes (11): Chat authorization hole (addParticipant bug, 2026-08-13), AuthContext (in-memory {userId, login, password}), Vite dev proxy (/api -> localhost:8080), SecurityConfig, ChatService, ChatService.getChatForParticipant authorization gate, HTTP Basic auth (no login endpoint), Bean (+3 more)

### Community 36 - "Backend/Frontend Plan Docs & Config Beans"
Cohesion: 0.27
Nodes (10): Java Backend Structure, Frontend Chat MVP Plan, Local vs AWS Component Mapping, RAG Pipeline Design, Support Chat Plan README, Architecture Overview, Design principles, Flow for a single message (current implementation) (+2 more)
...
Cohesion: 0.25
Nodes (7): 2026-09-03, App-side changes (small, enabling), CI, Manifests (`infra/k8s/`), Phase 3b: Kubernetes deploy layer (kubeadm on EC2) — lint-only skeleton, Still deferred (needs an AWS account), Terraform (`infra/terraform/`)
...
## Knowledge Gaps
- **398 isolated node(s):** `promptDelete`, `alwaysUpdateLinks`, `strictLineBreaks`, `vimMode`, `readableLineLength` (+393 more)
...
- **Why does `RAG Pipeline Design` connect `Backend/Frontend Plan Docs & Config Beans` to `ChatClient Config Bean`, `Chat Pipeline Orchestration & SSE`, `Groundedness Check & Intent JSON Validation`, `build.gradle`, `Intent Classification & Prompt Assembly`, `Knowledge Base & Deploy Workflow Docs`, `8-Stage RAG Pipeline Design`, `TextChunker Test`, `AWS Infrastructure Overview & TF Modules`, `Chat History Storage & Dialogue State`, `Minified Client Bundle Helpers C`, `Intent Definition Registry`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `Architecture Overview` connect `Backend/Frontend Plan Docs & Config Beans` to `AWS Infrastructure Overview & TF Modules`, `Chat Pipeline Orchestration & SSE`, `WebFlux Security & Auth Config`, `Validation Exception Handler`?**
