output "alb_dns_name" {
  description = "Public DNS name of the ingress ALB. Point the production hostname at this."
...
output "container_env" {
  description = <<-EOT
    ...
    modules/server/src/main/resources/application-prod.properties and the ConfigMap in
    infra/k8s/manifest-prod.yaml. Secret keys (POSTGRES_PASSWORD, CASSANDRA_USER,
