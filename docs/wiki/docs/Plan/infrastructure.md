...
| **RDS for PostgreSQL** | User accounts (JPA/Flyway-managed schema) |
...
- **Read caching / hot-intent caching** — reduces load on the Vector Store for repeated questions; the
  concrete store for this (Cassandra TTL rows vs. something else) is still undecided (see
