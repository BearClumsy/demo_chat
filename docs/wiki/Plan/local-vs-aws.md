...
(`ReactiveCassandraRepository`, `R2dbcRepository`, `ChatModel`/`VectorStore` — RAG is fully wired as of
Phase 2, see [rag-pipeline.md](rag-pipeline.md)) rather than a specific implementation, which is what
will allow local ↔ AWS switching to be purely config-driven once profiles are introduced.
