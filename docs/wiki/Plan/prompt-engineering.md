...
**Status:** implemented (Phase 1 input-side, Phase 2 output-side). Prompt assembly is `PromptBuilder`;
the output-side guardrail below is `ResponseValidator` + `GroundednessCheck` — see [rag-pipeline.md](rag-pipeline.md).
...
Implemented as designed above: `ResponseValidator.validate(answer, intent)` asks the same `ChatClient`
(not a separate NLI classifier) for a structured `GroundednessCheck` (`grounded`, `reasoning`),
checking the answer against the intent's `knowledgeSnippet`. On "no," `ChatPipelineService` swaps the
reply for the intent's `escalationFallback` text and sets `DialogueStatus.ESCALATED` — and the answer is
never written to the semantic cache (see [rag-pipeline.md](rag-pipeline.md)).
